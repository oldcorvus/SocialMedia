...
def user_profile(request, username ):
        ...
        if request.user == user:
            self_profile = True
        return render(request=request, template_name='account/profile.html', context={'user':user , 'self_profile':self_profile})

@login_required
def profile_edit(request, username):
    user = get_object_or_404(CustomUser, username=username)
    if request.method == 'POST':
        form = UserChangeForm(request.POST, instance=user, files=request.FILES)
        if form.is_valid():
            form.save()
